...
app.get('/dashboard', async (req, res) => {
    ...
    try {
        ...
        const deviceData = {
            ...
            id: device._id,
            pppoeUsername: device.VirtualParameters?.pppoeUsername?._value || 'N/A',
            pppoeUsername2: device.VirtualParameters?.pppoeUsername2?._value || 'N/A',
            ipAddress: device.VirtualParameters?.pppoeIP?._value || 'N/A',
            ssid: device.InternetGatewayDevice.LANDevice[1].WLANConfiguration[1].SSID._value || 'N/A',
            wlanPassword: device.VirtualParameters?.WlanPassword?._value || 'N/A',
            rxPower: device.VirtualParameters?.RXPower?._value || 'N/A',
            getponmode: device.VirtualParameters?.getponmode?._value || 'N/A',
            tags: device._tags || [],
            activedevices: device.VirtualParameters?.activedevices?._value || 'N/A',
            uptime: device.VirtualParameters?.getdeviceuptime?._value || 'N/A',
            temperature: device.VirtualParameters?.gettemp?._value || 'N/A'
        };
...
app.post('/update-wifi', async (req, res) => {
    ...
    try {
        ...
        if (password) {
            await axios({
                method: 'put',
                url: `${process.env.GENIEACS_URL}/devices/${deviceId}/parameters/VirtualParameters.WlanPassword`,
                data: {
                    value: password,
                    type: "xsd:string"
                },
        ...
        await axios.post(
            ...
            {
                name: "refreshObject",
                objectName: "VirtualParameters.WlanPassword"
            },
            {
                auth: {
                    username: process.env.GENIEACS_USERNAME,
...
app.post('/delete-tag', async (req, res) => {
    ...
    try {
        const { deviceId, tag } = req.body;
        const encodedDeviceId = encodeURIComponent(req.session.deviceId);
        const encodedTag = encodeURIComponent(tag);
...
// Endpoint untuk mengambil data WLAN
app.get('/device/wlan', async (req, res) => {
    if (!req.session.username || !req.session.deviceId) {
        return res.status(401).json({ error: 'Unauthorized' });
    }
    ...
    try {
        const deviceId = encodeURIComponent(req.session.deviceId);
        const response = await axios.get(`${process.env.GENIEACS_URL}/devices/${deviceId}`, {
            auth: {
                username: process.env.GENIEACS_USERNAME,
                password: process.env.GENIEACS_PASSWORD
            }
        ...
        const wlanData = {
            ssid: device?.InternetGatewayDevice?.LANDevice?.[1]?.WLANConfiguration?.[1]?.SSID?._value || 'N/A',
            wlanPassword: device?.InternetGatewayDevice?.LANDevice?.[1]?.WLANConfiguration?.[1]?.PreSharedKey?.[1]?.KeyPassphrase?._value || 'N/A',
            status: device?.InternetGatewayDevice?.LANDevice?.[1]?.WLANConfiguration?.[1]?.Status?._value || 'N/A',
            channel: device?.InternetGatewayDevice?.LANDevice?.[1]?.WLANConfiguration?.[1]?.Channel?._value || 'N/A',
            mode: device?.InternetGatewayDevice?.LANDevice?.[1]?.WLANConfiguration?.[1]?.Standard?._value || 'N/A'
        };

        res.json({ success: true, wlanData });
    } catch (error) {
        console.error('Error fetching WLAN data:', error);
        res.status(500).json({ error: 'Gagal mengambil data WLAN' });
    }
});
// Logout route
